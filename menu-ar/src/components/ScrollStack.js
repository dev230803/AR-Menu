...
const ScrollStack = ({
  ...
  const calculateProgress = useCallback((scrollLeft, start, end) => {
    if (scrollLeft < start) return 0;
    if (scrollLeft > end) return 1;
    return (scrollLeft - start) / (end - start);
  }, []);

  const parsePercentage = useCallback((value, containerWidth) => {
    if (typeof value === "string" && value.includes("%")) {
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const { scrollLeft, containerWidth, scrollContainer } = getScrollData();
    ...
    const endElementLeft = endElement ? getElementOffset(endElement) : 0;

    cardsRef.current.forEach((card, i) => {
      if (!card) return;

      const cardLeft = getElementOffset(card);
      ...
      if (blurAmount) {
        let topCardIndex = 0;
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardLeft = getElementOffset(cardsRef.current[j]);
          const jTriggerStart =
            jCardLeft - stackPositionPx - itemStackDistance * j;
      ...
      if (isPinned) {
        translateX =
          scrollLeft - cardLeft + stackPositionPx + itemStackDistance * i;
      ...
      if (i === cardsRef.current.length - 1) {
        const isInView = scrollLeft >= pinStart && scrollLeft <= pinEnd;
        if (isInView && !stackCompletedRef.current) {
          stackCompletedRef.current = true;
          onStackComplete?.();
        } else if (!isInView && stackCompletedRef.current) {
  ...
  const setupLenis = useCallback(() => {
    // Lenis configuration updated for horizontal orientation
    const lenisOptions = {
      orientation: "horizontal", // Crucial for horizontal scrolling
  ...
  useLayoutEffect(() => {
    ...
    cards.forEach((card, i) => {
      if (i < cards.length - 1) {
        card.style.marginRight = `${itemDistance}px`; // Changed from marginBottom
      }
      card.style.willChange = "transform, filter";
      card.style.transformOrigin = "left center"; // Changed from top center
      card.style.backfaceVisibility = "hidden";
      card.style.transform = "translateZ(0)";
      card.style.webkitTransform = "translateZ(0)";
      card.style.perspective = "1000px";
